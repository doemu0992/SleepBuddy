...
> **Selbstheilung: Fehler-Flag + Ergebnis-Watchdog (bindend, nachtbelegt):** Der Analyzer kann früh sterben („Error during analysis") und der Flag-Rebuild allein griff in einer realen Nacht **nie** (FEHLER 4 s nach Start geloggt, danach nachtlang stumm, kein REBUILD). Deshalb zweigleisig in `analyze()`: Rebuild bei gesetztem Fehler-Flag **oder** wenn seit `max(lastResultDate, lastAnalyzerRebuild)` ≥ 120 s **keine** ML-Ergebnisse kamen, obwohl Buffer fließen (Ergebnisse kommen normal < 1 s). `lastResultDate` wird in `didProduce` und `start()` gesetzt; der Rebuild-Grund („Fehler-Flag" / „Watchdog Xs ohne Ergebnis") steht im MLLog.
>
> **Software-Gain für den ML-Pfad (bindend, ADAPTIV):** Die AVAudioSession läuft im `.measurement`-Modus (AGC aus) für eine saubere Atemanalyse → sehr niedriger Eingangspegel. Auf der Matratze zusätzlich gedämpft. Ohne Verstärkung erreicht `SNClassifySoundRequest` für **keine** Klasse die Konfidenz → es werden **gar keine** Geräusche erkannt. Daher wird in `analyze(buffer:time:)` eine **gain-verstärkte Kopie** an den Analyzer gegeben; das rohe Signal bleibt für die Atem-/Amplitudenanalyse unberührt. **Der Gain ist adaptiv (nachtbelegt):** Spielt der Sonar-Ton, senkt die Mikrofon-Hardware die Eingangsempfindlichkeit um ~10–12 dB (Boden 15–25 dB statt 27–45 dB) — mit dem alten Fixwert ×8 brachen alle Konfidenzen ein und eine Nacht mit echtem Schnarchen lieferte **null** Erkennungen. Jetzt: Decay-Min-Tracker auf dem Ruhe-RMS (`floorRMS`, fällt sofort, steigt ~2 %/s — Events ziehen ihn nicht runter, keine AGC-Wirkung), `adaptiveGain = clamp(0.008 / floorRMS, 4…48)`. Bei ~30-dB-Boden ≙ ×8 (altes Verhalten ohne Sonar unverändert), bei Sonar-Boden ≙ ×25.
>
> **Weiche Sättigung statt Hard-Clip (bindend, datenbelegt):** `applyGain` sättigt mit **`vvtanhf` (tanh)**, nicht mit `vDSP_vclip`. Hard-Clipping erzeugte bei lauten Transienten (v.a. **Hundegebell**) harsche Oberwellen, die Apples Modell fälschlich Richtung `snoring` schoben (real beobachtet: Hund nur bei sehr lautem/nahem Audio korrekt als `dog` erkannt, nachts als Schnarchen gelabelt). tanh hält leise Signale linear (`tanh(x)≈x`, keine Empfindlichkeitseinbuße) und begrenzt laute weich → sauberere Klassifikation. Gain ggf. anhand echter Nächte nachjustieren.
...
>
> **Atem-basiertes Rand-Wach (`applyBreathingEdgeWake`, bindend — Sleep-Cycle-Methodik):** Ruhiges Wachliegen ist für Bewegung/Audio unsichtbar und HR kann fehlen — aber die **Atemrate** verrät es (real belegt: 20/min wach im Bett vs. Nacht-Median 15/min). TrainingSamples speichern die **beste** Atemquelle (Sensor/Sonar vor Audio). Pro-Minute-Atemrate, Schlaf-Kern-Median als Baseline, Schwelle Median + 3/min. **Abend:** Wach bis zur letzten schnellen Minute in den ersten 75 min (≥ 3 schnelle Minuten; verifiziert: 58-min-Abend-Wach korrekt erkannt). **Morgen (kritisch):** NUR ein **zusammenhängender** schneller Block, der bis zum Tracking-Ende reicht, rückwärts gescannt, ≥ 5 gemessene schnelle Minuten — die erste klar langsame Minute bricht ab. **Niemals** „erste schnelle Minute im Fenster" (REM atmet ebenfalls schnell → 50 min Falsch-Wach ab der Morgen-REM-Phase, real beobachtet). Erweitert Wach nur an den Rändern, löscht nie Schlaf in der Nachtmitte. Läuft in `stopTracking` und im Neuberechnen-Batch.
...
> Bewegung ist das **zuverlässigste** Wach-Signal (Umdrehen, Aufstehen, Unruhe). `applyMovementWake` nutzt die Pro-Minute-`movementIntensity` aus den TrainingSamples: **anhaltend** erhöhte Bewegung (> `elevated` für **≥ 3 min**) → `.awake` (via `markAwake`-Splitting).
>
> **Kein Einzel-Spike-Wach mehr (bindend, nutzerbelegt):** Ein **einzelner** (1–2 min) Bewegungs-Spike ist **Umdrehen**, kein Aufwachen — die alte Regel (`runLen >= 2 || spike > strong`) zeichnete über die ganze Nacht dünne Falsch-Wach-Streifen (real gemeldet: „nie wach mitten in der Nacht, nur abends/morgens"). **Aufstehen** (Toilette) dauert ohnehin mehrere Minuten und wird von der ≥ 3-min-Regel weiter erfasst. Zusätzlich entfernt ein **Aufräum-Pass am Ende** von `applyMovementWake` **kurze Wach-Inseln** (< 10 min), die **beidseitig** von Schlaf umgeben sind und **nicht** durch anhaltende Bewegung (< 3 erhöhte Minuten) gestützt werden — der Live-Klassifikator committet solche aus einem Momentan-Spike (Bewegung/Amplitude). Sie werden in die umgebende Schlafphase absorbiert, dann `mergeAdjacentSamePhases`. **Abend-/Morgen-Wach am Rand** (zusammenhängend, bzw. erste/letzte Phase) wird **nie** angetastet.
>
> **Roh-Minuten zählen, nicht die Glättung (bindend, kritisch):** Alle „anhaltend"-Prüfungen (`rawElev >= 3` im Run, `active` im 10-min-Fenster, `elevMin` im Aufräum-Pass) zählen auf **`rawMove`** (Pro-Minute-Rohwert), **niemals** auf der nachbar-geglätteten `moveByMin`. Die ±2-min-Glättung verschmiert einen **einzelnen** Umdreh-Spike auf 3–5 „erhöhte" Minuten — auf der geglätteten Kurve bestand damit jeder Umdreher die ≥ 3-min-Regel und der Aufräum-Pass hielt die Falsch-Insel für bewegungsgestützt (real beobachtet: Wach blieb nach Neuberechnung bei ~19 %). Die Glättung dient nur der **Run-Bildung** (zusammenhängende Fenster), nie dem Dauer-Beleg.
>
> **Intermittierende Unruhe (bindend, „hin und her wälzen"):** Umherwälzen ist oft **nicht durchgängig** (drehen, 30–60 s still, wieder drehen) — die Sustained-Run-Prüfung verpasst das. Zusätzlicher Pass: gleitendes **10-min-Fenster**; sind ≥ 6 Minuten darin erhöht (> `elevated`) **UND** mindestens ein echter starker Spike (> `strong`) drin, gilt das ganze Fenster als unruhig → `.awake`. Fängt „die ganze Nacht hin und her" ab, ohne sporadisches Umdrehen fälschlich als Stunden-Wach zu markieren. `movementIntensity` selbst ist peak-sensitiv (`√var×25 + peak×3`), damit kurze Rollbewegungen nicht im 30-s-Mittel untergehen. **Bewusst KEIN BCG-Null-Heuristik** (die markierte fälschlich ruhigen Schlaf als wach). Völlig ruhiges Wachliegen bleibt prinzipbedingt unerkennbar. Läuft in `stopTracking` und im „neu berechnen"-Batch. Die Plausibilitäts-Korrektur **merged `.awake` nie weg** (der Aufräum-Pass in `applyMovementWake` ist bewegungs-informiert und dafür zuständig).
...
> **Sonar-Puls (EXPERIMENTELL):** `heartRate(from:rate:)` schätzt den Puls (40–110 BPM) aus der clutter-bereinigten Bewegungskomponente (Autokorrelation, strengere Prominenz-Gates als Atmung, nur bei Bewegung < 0.3). Geht in die Nacht-CSV (`puls_bpm`, **letzte** Spalte — Parser-Indizes bleiben gültig, Rohwert), den Live-Log und **gegated** in `SonarFeatures.heartRateBPM`.
>
> **NIEMALS MA-Hochpass im Puls-Pfad (bindend, nachtbelegt):** Der frühere „1,2-s-Moving-Average abziehen"-Hochpass erzeugte ein ACF-Artefakt bei der **halben Fensterlänge** (~30 Samples ≙ ~96 BPM) — eine reale Nacht lockte 2034/2165 Fenster konstant auf 93–100 BPM, während der echte Puls bei 55–70 lag; über den Fusion-Lücken-Füller verschmutzte das die gespeicherte Puls-Reihe. Stattdessen **Bandpass 0,5–2,5 Hz aus zwei Ein-Pol-IIR-Filtern** (keine Lag-Signatur). Zusätzlich **Stabilitäts-Gate** vor der Fusion: `heartRateBPM` wird nur emittiert, wenn ≥ 2 der letzten 3 Roh-Messungen innerhalb ±8 BPM liegen (`recentSonarHR`, in `reset()` geleert).
>
> **HR-Fusion (konservativ, bindend):** Prioritität im Minuten-Sampler: **Watch → BCG → Sonar**. Sonar stützt das BCG in zwei engen Fällen: (1) **Oberwellen-Korrektur** — zeigt das unabhängige Sonar denselben Puls bei ~halber BCG-Höhe (`bcgHR ≈ 1.7–2.3 × sonarHR`), ist der BCG-Wert ein harmonischer Sprung → Sonar-Wert wird gespeichert. (2) **Lücken-Füller mit Mess-Anker (bindend, nachtbelegt)** — BCG ohne Lock + gültiges frisches Sonar (< 90 s, 40–110 BPM) → Sonar-Wert statt 0, aber **nur wenn er ±15 BPM am letzten echt gemessenen Watch-/BCG-Wert liegt** (`lastGoodHRForSonar`, in `startTracking` genullt). Der rohe Sonar-Puls kann **stabil** auf einer Lag-Artefakt-Familie locken (real: 93/96/100/103 in 94 % der Nachttisch-Fenster) — das ±8-Stabilitäts-Gate hält das nicht auf; ohne Anker darf Sonar-HR nie frei in die gespeicherte Reihe laufen. Bei Übereinstimmung oder ohne Sonar-Lock bleibt BCG unangetastet; die Watch wird nie überstimmt.

**Tracking-Integration (opt-in, `sonar_enabled`, Standard AUS, bindend):**

> **Eine Engine (bindend):** Zwei Mikrofon-Engines gleichzeitig gehen nicht. Bei aktivem Sonar spielt **`AudioAnalysisService`s** Engine zusätzlich den 19-kHz-Ton (`sonarTonePlayer`) und die Session wird `.playAndRecord`/`.measurement` (kein BluetoothHFP). Jeder Roh-Buffer geht an `sonar.feedExternal(buffer)`; für die restliche Analyse (Amplitude/dB, ML, Clips) wird der Ton per **Notch-Biquad (`notchedCopy`, RBJ, f0=19 kHz, Q=8) + kaskadiertem 11-kHz-Lowpass (`configureLowpass`)** entfernt — sonst würde der Dauerton die Lautstärke-/Geräuschmessung verfälschen. **Der Lowpass ist Pflicht (nutzerbelegt):** Der Notch allein ist zu schmal — Rest-Ton/Verzerrungen hoben real den Pegel die ganze Nacht auf ~45 dB, alles wurde als „Geräusch" erkannt und die ML-Klassifikation fand nichts mehr. Schlafgeräusche liegen < 8 kHz; alles über 11 kHz ist für die Analyse Müll. `SonarService` läuft dann **ohne eigene Engine** (`resetForTracking`/`feedExternal`).
>
...
>
> **Ton-Amplitude 0.35 — NIE global anheben (bindend, nachtbelegt):** Ein globaler Boost auf 0.50 hat die validierte Referenz-Nacht zerstört: Der lautere Ton drückt die Mikrofon-Schutzdämpfung stärker (Boden 15 dB), Atem-Lock 96 % → 27 % (Reg ~25 %), ML taub, Tiefschlaf 1 min (Pseudo-REM-Atmung degradierte nachtweit), 70 min Falsch-Abend-Wach aus der rausch-selektierten Reihe. Schwache Empfänger (Pegel ~0.0002) bekommen stattdessen die **adaptive Stufen-Anhebung** (`sonarRampTimer`: alle 90 s, Pegel < 0.0006 → +0.15, max 0.8). Dazu zwei Qualitäts-Gates: Sonar-Atmung überschreibt MotionFeatures nur bei Regularität ≥ 0.3, und `applyBreathingEdgeWake` greift nur bei ≥ 50 % Kern-Abdeckung.

**Lautstärke-Floor (bindend, gerätebelegt):** Der Sonar-Ton läuft über die Medien-Wiedergabe — steht die Medienlautstärke auf 0/leise, ist der Ton stumm und das Sonar die ganze Nacht blind (real beobachtet: Zweitgerät, Pegel 0.0000 in allen Fenstern trotz korrektem Build). `ensureSonarVolume()` hebt die Systemlautstärke beim Sonar-Start auf mindestens 0.6 an (MPVolumeView-Technik wie beim Wecker; nur anheben, nie absenken).
...
> **Rückwirkende Reparatur strukturloser Nächte (`rebuildPhasesFromSamples`, bindend):** „Schlafphasen neu berechnen" baut die Phasen aus den `TrainingSample`s neu auf. Erkennt es eine **strukturlose** Nacht (≥ 20 Schlafminuten, aber < 5 % Tief/REM — die Signatur einer Onset-losen Nacht, bei der das 90-min-Modell nie lief), rekonstruiert es den Onset aus der Pro-Minute-Bewegung (erster ≥ 5-min-Block „nicht wach + Bewegung < 0.30"; **Nachhaltigkeits-Gate:** Kandidat zählt nur, wenn die 45 min danach ≥ 70 % schlaf-kompatibel sind — eine ruhige Insel im abendlichen Wachliegen setzt den Onset sonst zu früh, real beobachtet: 1m Wach → 15m „Schlaf" → 22m Wach) und **legt das Zyklusmodell rückwirkend** über die ruhige Strecke (Zonen A/B/C aus der erkannten Zykluslänge; echte Wach-Minuten aus dem Bewegungssignal bleiben erhalten). Setzt `session.sleepOnsetDate`, damit die nachfolgenden Korrektur-Pässe (`applyCycleRemRefinement`, `detectCycleLength`) greifen. **Normale Nächte** (mit echtem Tief/REM in den Live-Labels) werden **nicht angetastet** — die Overlay-Logik ist rein ein Fallback.
...
> **Weckzeit beim Arm verankern (bindend):** `arm()` berechnet `armedEarliest`/`armedLatest` als das **nächste Vorkommen** der Weck-Tageszeiten **ab Tracking-Start** (`nextOccurrence`) — die Weckzeit liegt damit **immer in der Zukunft**. `isPastLatest`/`isInsideWindow` vergleichen `now` gegen diese festen Datumswerte. Das ersetzt die frühere 12-h-Heuristik (`normalizedWindowTime`, bleibt als Fallback), die bei einem **Test tagsüber** versagte: um 15:00 lag die Weckzeit 06:25 nur ~8,5 h in der Vergangenheit (< 12 h) → nicht auf morgen geschoben → `isPastLatest` == true → Wecker feuerte sofort nach ~20 s. Mit Verankerung wird 06:25 korrekt auf **morgen** gelegt. `disarm()` löscht die verankerten Zeiten.
...
>
> **Auswertungs-Werkzeuge (bindend):** (a) **Pass-Bilanz** — existiert eine datierte Watch-Referenz (`WatchRef-<startTS>.csv`, vom Watch-Vergleich als „Golden Night" gespeichert, nicht rotiert), misst `reapplyPhaseCorrections` die Phasen-Übereinstimmung **nach jedem Pass** und schreibt sie als eine Zeile ins PassAudit — zeigt sofort, welcher Pass trägt/schadet. (b) **Watch-Vergleich erweitert**: Cohen's Kappa, Sensitivität pro Phase, klinische Aggregate (Einschlaflatenz/WASO/REM-Latenz/Aufwachungen Watch vs. App), Übereinstimmung stratifiziert nach Sonar-Lock. (c) **Gelernte Schlafarchitektur**: jede Watch-Nacht (≥ 200 min Überlappung) lernt per EMA `cal_watchDeepPct`/`cal_watchRemPct`/`cal_watchRemLatencyMin`; ab 3 Watch-Nächten ersetzen sie die 22 %/25 %-Budgets in `applyDeepRedistribution` (geklemmt 10–28 %/15–30 %). (d) **`AlgoVersion.current`** (FeatureNightLog.swift) bei jeder Tuning-Änderung hochzählen — steht im FeatureLog-Header + DebugInfo.
>
...
> **Replay-Vollständigkeit (bindend):** Das Offline-Replay muss mit **exakt** den Daten rechnen, die die Geräte-Pässe sehen — sonst validiert es die falsche Welt (real passiert: RegularityREM im Replay +4 Pkt mit Sonar-Reg, auf dem Gerät −8 Pkt mit dem TrainingSample-Mix aus Sonar-Reg + Audio-1.0-Pins). Daher: (1) Das **Debug-Paket** („Debug-Paket teilen") exportiert zusätzlich **`TrainingSamples.csv`** (die maßgebliche Eingabe aller Korrektur-Pässe: amp/ampVar/atem/reg/bewegung/schnarchen/label/korrigiert pro 30 s) und **`SessionState.json`** (Session-Zeiten, sleepOnsetDate, alarmFiredDate, gespeicherte `heartRateSamples` inkl. Watch-Backfill, `noiseSamples`, `cal_*`-Baselines). (2) **FeatureNightLog loggt das Sonar-gemergte Motion** (nicht `motionIn`) — atem_best/reg_best = das, was Klassifikator + TrainingSamples sehen. (3) Regularität ≥ 0.95 ist die **Audio-Pin-Signatur** (sättigt auf 1.0) und gilt in Retro-Pässen als „nicht gemessen". (4) `usageAwakeIntervals` werden beim Stopp in UserDefaults persistiert (`usageIntervals.<startTS>`), im Neuberechnen via `applyPersistedUsageAwake` erneut angewendet und in `SessionState.json` exportiert. (5) **`SoundEvents.csv`** (Zeit/Typ/Dauer/dB/Konfidenz/Korrektur) ist Teil des Debug-Pakets.
